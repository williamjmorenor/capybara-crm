...
<div class="d-flex justify-content-between align-items-center mb-3">
    ...
    <div class="d-flex gap-2">
        <a href="/leads/<?= $lead['id'] ?>/edit" class="btn btn-outline-primary btn-sm"><i class="bi bi-pencil me-1"></i>Edit</a>
        <a href="/leads" class="btn btn-outline-secondary btn-sm"><i class="bi bi-arrow-left me-1"></i>Back</a>
    </div>
</div>
...
<div class="row g-3">
    <div class="col-md-5">
        <div class="card border-0 shadow-sm mb-3">
            <div class="card-body">
                <h6 class="fw-semibold text-muted text-uppercase small mb-3">Lead Details</h6>
                <dl class="row mb-0">
                    ...
                    <dd class="col-sm-7"><span class="badge bg-light text-dark text-capitalize"><?= esc($lead['source']) ?></span></dd>
                    <dt class="col-sm-5 text-muted">Status</dt>
                    <dd class="col-sm-7"><span class="badge badge-status-<?= $lead['status'] ?>"><?= ucfirst($lead['status']) ?></span></dd>
                    <dt class="col-sm-5 text-muted">Est. Value</dt>
        ...
        <div class="card border-0 shadow-sm border-success">
            <div class="card-body text-center">
                ...
                <p class="mb-3 text-muted small">Convert this lead into a contact to move them forward in the pipeline.</p>
                <form method="post" action="/leads/<?= $lead['id'] ?>/convert" onsubmit="return confirm('Convert this lead to a contact?')">
                    <?= csrf_field() ?>
                    <button type="submit" class="btn btn-success btn-sm">
                        <i class="bi bi-arrow-right-circle me-1"></i>Convert to Contact
                    </button>
                </form>
            </div>
        </div>
