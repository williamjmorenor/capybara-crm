...
<div class="d-flex justify-content-between align-items-center mb-3">
    <h5 class="fw-semibold mb-0"><i class="bi bi-pencil me-2 text-primary"></i><?= lang('Crm.edit_contact') ?></h5>
    <a href="/contacts/<?= $contact['id'] ?>" class="btn btn-outline-secondary btn-sm"><i class="bi bi-arrow-left me-1"></i><?= lang('Crm.back') ?></a>
</div>
    ...
    <div class="card-body">
        <form method="post" action="/contacts/<?= $contact['id'] ?>/edit" novalidate>
            ...
            <div class="row g-3 mb-3">
                ...
                <div class="col-sm-6">
                    ...
                    <select class="form-select" name="status" required>
                        <option value="active" <?= old('status', $contact['status']) === 'active' ? 'selected' : '' ?>><?= lang('Crm.status_active') ?></option>
                        <option value="inactive" <?= old('status', $contact['status']) === 'inactive' ? 'selected' : '' ?>><?= lang('Crm.status_inactive') ?></option>
                    </select>
                </div>
            </div>
            <div class="mb-4">
            ...
            <div class="d-flex gap-2">
                ...
                <a href="/contacts/<?= $contact['id'] ?>" class="btn btn-outline-secondary"><?= lang('Crm.cancel') ?></a>
            </div>
        </form>
    </div>
</div>
