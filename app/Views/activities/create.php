...
<div class="d-flex justify-content-between align-items-center mb-3">
    <h5 class="fw-semibold mb-0"><i class="bi bi-calendar-plus me-2 text-info"></i><?= lang('Crm.new_activity') ?></h5>
                ...
                <div class="col-sm-6">
                    ...
                    <select class="form-select" name="type" required>
                        <?php foreach (['call', 'email', 'meeting', 'note'] as $t): ?>
                            <option value="<?= $t ?>" <?= old('type', $type ?? '') === $t ? 'selected' : '' ?>><?= lang('Crm.type_' . $t) ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
                <div class="col-sm-6">
                ...
                <div class="col-sm-6">
                    ...
                    <select class="form-select" name="related_type">
                        ...
                        <?php foreach (['lead', 'contact', 'opportunity'] as $rt): ?>
                            <option value="<?= $rt ?>" <?= old('related_type', $related_type ?? '') === $rt ? 'selected' : '' ?>><?= lang('Crm.related_' . $rt) ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
                <div class="col-sm-6">
                    ...
                    <input type="number" class="form-control" name="related_id"
                           value="<?= esc(old('related_id', $related_id ?? '')) ?>" min="1" placeholder="<?= lang('Crm.related_id_placeholder') ?>">
                </div>
            </div>

            <div class="d-flex gap-2">
                <button type="submit" class="btn btn-primary"><i class="bi bi-check-lg me-1"></i><?= lang('Crm.create_activity') ?></button>
