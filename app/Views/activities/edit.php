...
<div class="d-flex justify-content-between align-items-center mb-3">
    <h5 class="fw-semibold mb-0"><i class="bi bi-pencil me-2 text-info"></i>Edit Activity</h5>
                ...
                <div class="col-sm-6">
                    ...
                    <select class="form-select" name="type" required>
                        <?php foreach (['call', 'email', 'meeting', 'note'] as $t): ?>
                            <option value="<?= $t ?>" <?= old('type', $activity['type']) === $t ? 'selected' : '' ?>><?= ucfirst($t) ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
                <div class="col-sm-6">
                ...
                <div class="col-sm-6">
                    ...
                    <select class="form-select" name="related_type">
                        ...
                        <?php foreach (['lead', 'contact', 'opportunity'] as $rt): ?>
                            <option value="<?= $rt ?>" <?= old('related_type', $activity['related_type']) === $rt ? 'selected' : '' ?>><?= ucfirst($rt) ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
                <div class="col-sm-6">
